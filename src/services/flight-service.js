...
async function createFlight(data) {
    try {
        ...
        if(isNotValid){
            const error = new Error("Departure time must be greater than arrival time");
    ...
    try {
        ...
        if(query.trips){
            ...
            if(departureAirportId === arrivalAirportId){
                const error = new Error("Departure and Arrival destination cannot be same");
        ...
        if(query.price){
            if(!(/^\d{1,7}(\.\d{1,2})?-\d{1,7}(\.\d{1,2})?$/.test(query.price))){
                const error = new Error("Price must be a valid number with up to 2 decimal places");
                error.name = "CustomError"; // ✅ Custom name
                throw error;
            }
